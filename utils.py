...
# Splits the given dataset into a set of training and testing data
def split_train_dev_test(data, labels, test_size, dev_size = 0 ):
    X_Train_Dev, X_Test, Y_Train_Dev, Y_Test = train_test_split(data, labels, test_size=test_size, shuffle=False)
    if(dev_size > 0):
        #Splitting the Initial  data into 
        X_Train, X_Dev, Y_Train, Y_Dev = train_test_split(X_Train_Dev, Y_Train_Dev, dev_size, shuffle=False)
...
def predict_and_eval(model, X_test, y_test):
    ...
    # Evaluate model
    evaluate_model(y_test, predicted, model)
